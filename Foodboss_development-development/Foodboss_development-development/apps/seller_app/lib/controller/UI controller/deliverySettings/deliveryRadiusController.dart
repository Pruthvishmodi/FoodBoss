...
class MapDeliveryController extends GetxController {
  ...
  changeZoomValue(double value) async {
    ...
    printError(info: zoomLevel.toString());
  ...
  addDelivery(int id) {
    if (rangesDelivery.value.length <= 2) {
      rangesDelivery.value[id] = DeliveryModel(
          price: "price",
          range: ranges[sliderValue.value].toString(),
  ...
  setLocation() async {
    var location = await AuthCall.getLocation();

    initialcameraposition.value = LatLng(location[0], location[1]);
  }
